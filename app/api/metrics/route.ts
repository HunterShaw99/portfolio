// Next.js API route for Prometheus metrics
...
${Array.from(appMetrics.requestsByPath.entries()).map(([path, count]) => {
...
${Array.from(appMetrics.requestsByMethod.entries()).map(([method, count]) => {
...
${Array.from(appMetrics.requestsByStatus.entries()).map(([status, count]) => {
...
// Optional: You can also add a POST endpoint to increment error count
export async function POST(request: NextRequest) {
    const appMetrics = getCurrentMetrics()
    appMetrics.errorCount++
    return NextResponse.json({ status: 'error recorded', errorCount: appMetrics.errorCount })
}